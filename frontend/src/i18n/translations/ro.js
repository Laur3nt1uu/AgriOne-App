export default {
  nav: {
    features: "Funcționalități",
    howItWorks: "Cum Funcționează",
    pricing: "Prețuri",
    about: "Despre Noi",
    faq: "Întrebări",
    getStarted: "Începe Acum",
    signIn: "Intră în Cont",
    goToDashboard: "Deschide Aplicația",
    mega: {
      platform: "Platformă",
      platformDesc: "Totul despre ce oferim",
      solutions: "Soluții",
      solutionsDesc: "Pentru orice tip de fermă",
      resources: "Resurse",
      resourcesDesc: "Învață și prosperă",
      monitoring: {
        title: "Monitorizare în Timp Real",
        desc: "Date live de la senzorii din câmp",
      },
      alerts: {
        title: "Alerte Inteligente",
        desc: "Notificări SMS și email când ceva nu e în regulă",
      },
      mapping: {
        title: "Hărțile Fermei",
        desc: "Parcelele tale vizibile pe hartă cu coordonate GPS",
      },
      financial: {
        title: "Gestiune Financiară",
        desc: "Cheltuieli și venituri înregistrate pe fiecare cultură",
      },
      reports: {
        title: "Rapoarte PDF",
        desc: "Exportă date detaliate pentru fiecare teren",
      },
      analytics: {
        title: "Analize Avansate",
        desc: "Grafice, tendințe și decizii bazate pe date",
      },
      smallFarms: {
        title: "Ferme Mici",
        desc: "Sub 10 hectare — start rapid și simplu",
      },
      mediumFarms: {
        title: "Ferme Medii",
        desc: "10-100 hectare — monitorizare completă",
      },
      largeFarms: {
        title: "Ferme Mari & Cooperative",
        desc: "100+ hectare — soluții enterprise",
      },
      crops: {
        title: "Culturi Diverse",
        desc: "Grâu, porumb, floarea soarelui și altele",
      },
      docs: {
        title: "Documentație",
        desc: "Ghiduri de instalare și configurare",
      },
      blog: {
        title: "Blog",
        desc: "Articole despre agricultura inteligentă",
      },
      helpCenter: {
        title: "Centru de Ajutor",
        desc: "Întrebări frecvente și suport tehnic",
      },
      community: {
        title: "Comunitate",
        desc: "Alătură-te fermierilor din România",
      },
      featuredPost: "Articol Recomandat",
      featuredPostTitle: "Cum IoT transformă agricultura în România",
      featuredPostDesc: "Descoperă cum senzorii inteligenți ajută fermierii să economisească apă și să crească recolta cu până la 30%.",
      readMore: "Citește mai mult",
    },
  },

  hero: {
    badge: "Platformă Agricolă Inteligentă",
    headline: "Ferma ta,",
    headlineHighlight: "mai deșteaptă",
    subheadline:
      "Ții evidența terenurilor, vezi ce spun senzorii în timp real și iei decizii mai bune pentru recoltă. AgriOne pune totul într-un singur loc.",
    ctaPrimary: "Deschide Platforma",
    ctaPrimaryAuthed: "Deschide Aplicația",
    ctaSecondary: "Află mai multe",
    scrollToExplore: "Continuă să citești",
    stats: {
      farms: "Ferme Monitorizate",
      sensors: "Senzori Activi",
      uptime: "Funcționare Continuă",
    },
  },

  features: {
    badge: "Funcționalități",
    title: "Totul într-un singur loc, ca să-ți",
    titleHighlight: "ușurezi",
    titleEnd: "munca",
    subtitle:
      "De la senzorii din câmp până la evidența financiară — AgriOne te ajută să ai o imagine completă asupra fermei și să iei decizii mai bune.",
    monitoring: {
      title: "Monitorizare în Timp Real",
      description: "Vezi umiditatea și temperatura din sol non-stop",
      details:
        "Datele de la senzori vin direct pe dashboard. Vezi ce se întâmplă pe fiecare teren, fără să trebuiască să mergi în câmp de fiecare dată.",
    },
    alerts: {
      title: "Alerte și Notificări",
      description: "Primești mesaj pe email sau SMS când ceva nu e în regulă",
      details:
        "Setezi tu pragurile — dacă e prea uscat, prea cald sau prea umed, primești alertă. Așa nu mai ratezi momentul potrivit pentru irigat sau alte intervenții.",
    },
    mapping: {
      title: "Harta Terenurilor",
      description: "Toate parcelele tale, vizibile pe o hartă interactivă",
      details:
        "Trasezi limitele terenurilor cu GPS, vezi suprafețele, și ai totul organizat vizual. Ușor de înțeles, fără hârtii și caiete.",
    },
    financial: {
      title: "Evidența Financiară",
      description: "Ții socoteala cheltuielilor și veniturilor pe fiecare cultură",
      details:
        "Știi exact cât ai investit și cât ai scos pe fiecare teren. Calculezi randamentul și vezi unde merită să investești mai mult.",
    },
    learnMore: "Află mai mult",
    bottomCta: "Vrei să vezi cum funcționează concret?",
    seeHowItWorks: "Vezi Pașii",
  },

  dashboardPreview: {
    badge: "Vizualizare",
    title: "Uite cum vezi ferma",
    titleHighlight: "în timp real",
    subtitle:
      "Un dashboard simplu care îți arată tot ce trebuie — de la senzorii din câmp la situația financiară, totul pe un singur ecran.",
    kpis: {
      farms: "Ferme Conectate",
      sensors: "Senzori Activi",
      alerts: "Alerte Trimise",
      uptime: "Disponibilitate",
    },
    mockLabels: {
      dashboard: "Dashboard",
      overview: "Sumar",
      liveData: "Date Live",
      soilMoisture: "Umiditate Sol",
      temperature: "Temperatură",
      humidity: "Umiditate Aer",
      alerts: "Alerte Active",
    },
  },

  howItWorks: {
    badge: "Cum Funcționează",
    title: "Pornești în",
    titleHighlight: "3 pași",
    subtitle:
      "Nu e complicat deloc. Instalezi senzorii, configurezi alertele și gata — aplicația face restul.",
    steps: {
      install: {
        title: "Pui Senzorii",
        description: "Îi conectezi la terenul tău",
        details:
          "Montezi senzorii IoT în câmp și ei încep să colecteze date automat: umiditate sol, temperatură, umiditatea aerului.",
        duration: "5 minute",
      },
      rules: {
        title: "Setezi Regulile",
        description: "Stabilești praguri pentru temperatură și umiditate",
        details:
          "Alegi tu limitele — de exemplu, dacă umiditatea scade sub 30%, primești alertă. Fiecare cultură poate avea reguli diferite.",
        duration: "10 minute",
      },
      monitor: {
        title: "Urmărești și Crești",
        description: "Folosești dashboard-ul ca să iei decizii mai bune",
        details:
          "Vezi grafice, tendințe și rapoarte clare. Pe parcurs, înveți ce funcționează și îți optimizezi recolta.",
        duration: "Continuu",
      },
    },
    bottomTitle: "Gata să încerci?",
    bottomSubtitle:
      "E gratuit să începi. Faci cont, adaugi terenurile și vezi cum arată dashboard-ul cu datele tale.",
    ctaPrimary: "Creează Cont Gratuit",
    ctaPrimaryAuthed: "Mergi la Dashboard",
    ctaSecondary: "Conectare",
    ctaSecondaryAuthed: "Vezi Profilul",
  },

  testimonials: {
    badge: "Testimoniale",
    title: "Ce zic",
    titleHighlight: "fermierii",
    subtitle:
      "Află de la fermieri reali care au încercat AgriOne și au văzut rezultate.",
    items: [
      {
        name: "Ion Popescu",
        quote:
          "Am pus doi senzori pe câmp și acum văd direct pe telefon dacă trebuie să irig. Simplu și fără bătăi de cap.",
        farm: "42 hectare, Brăila",
      },
      {
        name: "Maria Ionescu",
        quote:
          "Alertele m-au ajutat de câteva ori să nu ratez irigatul. Înainte ghiceam, acum știu sigur când e nevoie.",
        farm: "18 hectare, Cluj",
      },
      {
        name: "Andrei Moldovan",
        quote:
          "Credeam că astea-s chestii doar pentru fermele mari. Dar e ușor de folosit și nu costă mult să începi.",
        farm: "65 hectare, Timiș",
      },
      {
        name: "Elena Dumitrescu",
        quote:
          "Mi-a plăcut partea cu banii — văd clar cât am cheltuit și cât am scos pe fiecare cultură. Ajută mult la planificare.",
        farm: "28 hectare, Iași",
      },
    ],
    trustStats: {
      farms: "ferme conectate",
      rating: "notă medie",
      satisfaction: "mulțumiți",
    },
  },

  pricing: {
    badge: "Prețuri",
    title: "Planuri simple pentru",
    titleHighlight: "orice fermă",
    subtitle:
      "Începi gratuit, fără obligații. Dacă vrei mai mult, treci pe Pro. Prețuri clare, fără surprize.",
    mostPopular: "Recomandat",
    perMonth: "/lună",
    free: "Gratuit",
    custom: "La cerere",
    ctaStarter: "Începe Gratuit",
    ctaPro: "Alege Pro",
    ctaEnterprise: "Contactează-ne",
    ctaAuthed: "Deschide Aplicația",
    ctaCurrent: "Planul tău curent",
    ctaUpgrade: "Fă upgrade",
    ctaChangePlan: "Schimbă planul",
    tiers: {
      starter: {
        name: "Starter",
        price: "0 RON",
        description: "Ca să testezi platforma",
        features: [
          "Până la 2 terenuri",
          "Până la 5 senzori",
          "Monitorizare de bază",
          "Alerte pe email",
          "Dashboard simplu",
          "Suport comunitate",
        ],
      },
      pro: {
        name: "Pro",
        price: "49 RON",
        description: "Pentru fermieri care vor totul",
        features: [
          "Terenuri nelimitate",
          "Până la 50 senzori",
          "Analize detaliate",
          "Alerte SMS și email",
          "Rapoarte PDF",
          "Evidență financiară",
          "Suport prioritar",
        ],
      },
      enterprise: {
        name: "Enterprise",
        price: "La cerere",
        description: "Pentru ferme mari sau asociații",
        features: [
          "Senzori nelimitați",
          "Acces la API",
          "Integrări la cerere",
          "Manager de cont dedicat",
          "SLA garantat",
          "Instalare on-premise",
        ],
      },
    },
  },

  about: {
    badge: "Despre Noi",
    title: "Suntem o echipă de",
    titleHighlight: "dezvoltatori",
    subtitle:
      "AgriOne este creat de studenți pasionați de tehnologie și agricultură, cu experiență în dezvoltare software și senzori IoT. Misiunea noastră este să aducem agricultura României în era digitală.",
    mission: {
      label: "Cine suntem",
      text: "Suntem o echipă de dezvoltatori software cu background în tehnologii web moderne, IoT și baze de date. Avem experiență practică cu React, Node.js, senzori Arduino și platforme cloud. Proiectul nostru combină cunoștințele tehnice cu o problemă reală din agricultură.",
    },
    vision: {
      label: "Care este viziunea noastră",
      text: "Vrem să demonstrăm că tehnologia poate fi simplă și utilă pentru fermieri. Nu creăm doar o aplicație — construim punți între inovația tehnologică și nevoile concrete ale agriculturii românești. Fiecare feature este gândit să rezolve o problemă reală.",
    },
    thesisBadge: "Proiect de Licență - Ingineeri Software",
    thesisText:
      "AgriOne este proiectul nostru de licență în Ingineria Sistemelor - o platformă full-stack care demonstrează integrarea tehnologiilor web moderne (React, Node.js, PostgreSQL) cu hardware IoT pentru monitorizarea în timp real a condițiilor agricole.",
    values: {
      accessibility: {
        title: "Accesibilitate",
        description: "Nu trebuie să ai o fermă mare ca să beneficiezi de tehnologie. Am făcut totul cât mai simplu.",
      },
      simplicity: {
        title: "Simplitate",
        description: "Fără meniuri complicate. Intri, vezi datele, iei decizii. Atât.",
      },
      innovation: {
        title: "Tehnologie Modernă",
        description: "Senzori IoT, dashboard în timp real, alerte automate — toate într-o aplicație web.",
      },
      community: {
        title: "Pentru Comunitate",
        description: "Construim pentru fermieri din România, cu feedback de la ei și cu nevoile lor în minte.",
      },
    },
  },

  faq: {
    badge: "Întrebări Frecvente",
    title: "Ai întrebări?",
    titleHighlight: "Normal",
    subtitle:
      "Aici sunt răspunsurile la ce ne întreabă lumea cel mai des.",
    items: [
      {
        question: "Ce senzori îmi trebuie?",
        answer:
          "Poți începe cu senzori simpli de temperatură și umiditate sol — se găsesc de la 50-100 RON bucata. Platforma funcționează cu orice senzor care trimite date prin MQTT sau HTTP. Avem și ghiduri de conectare.",
      },
      {
        question: "Cât costă? Sunt costuri ascunse?",
        answer:
          "Planul Starter e gratis — ai monitorizare de bază pentru 2 terenuri și 5 senzori. Dacă vrei mai mult, Pro e 49 RON pe lună. Atât, nimic ascuns.",
      },
      {
        question: "Merge și pentru terenuri mici?",
        answer:
          "Da, chiar pentru asta am făcut-o. Fie că ai jumătate de hectar sau 100, platforma se potrivește. Nu trebuie să ai fermă mare ca să merite.",
      },
      {
        question: "Sunt datele mele în siguranță?",
        answer:
          "Da. Totul e criptat, respectăm GDPR, și nu dăm datele tale nimănui. E treaba ta ce faci cu ele.",
      },
      {
        question: "Trebuie să mă pricep la calculatoare?",
        answer:
          "Nu. Dacă știi să folosești un telefon, te descurci și cu AgriOne. Am făcut interfața cât mai simplă, și avem ghiduri dacă te blochezi.",
      },
      {
        question: "Pot să export datele?",
        answer:
          "Da, cu planul Pro poți exporta rapoarte PDF pentru fiecare teren — cu date de la senzori, grafice și situația financiară. Bune pentru dosare sau planificarea sezonului.",
      },
    ],
  },

  ctaBanner: {
    title: "Vrei să încerci?",
    subtitle:
      "Faci un cont gratuit, adaugi terenurile și vezi direct cum funcționează. Fără obligații.",
    ctaPrimary: "Creează Cont Gratuit",
    ctaPrimaryAuthed: "Mergi la Dashboard",
    ctaSecondary: "Contactează-ne",
    trust: {
      noCreditCard: "Fără card bancar",
      quickSetup: "Gata în 5 minute",
      cancelAnytime: "Renunți oricând",
    },
  },

  footer: {
    description:
      "AgriOne ajută fermierii să ia decizii mai bune cu date de la senzori IoT, hărți interactive și evidență financiară clară.",
    platform: "Platformă",
    company: "Companie",
    resources: "Resurse",
    legal: "Legal",
    links: {
      features: "Funcționalități",
      howItWorks: "Cum Funcționează",
      pricing: "Prețuri",
      documentation: "Documentație",
      aboutUs: "Despre Noi",
      careers: "Cariere",
      pressKit: "Kit de Presă",
      contact: "Contact",
      blog: "Blog",
      helpCenter: "Centru de Ajutor",
      community: "Comunitate",
      apiDocs: "Documentație API",
      privacyPolicy: "Confidențialitate",
      termsOfService: "Termeni și Condiții",
      cookiePolicy: "Cookie-uri",
      gdpr: "GDPR",
    },
    followUs: "Urmărește-ne:",
    stayUpdated: "Fii la curent",
    securePayments: "Plăți securizate prin",
    copyright: "Toate drepturile rezervate.",
    madeWith: "Făcut cu",
    inRomania: "în România",
    allSystemsOperational: "Totul funcționează",
  },

  plan: {
    title: "Gestionare Abonament",
    subtitle: "Alege planul care ți se potrivește",
    currentBadge: "Planul Curent",
    confirmTitle: "Confirmă Schimbarea",
    confirmMessage: "Ești pe cale să treci pe planul",
    confirmNote: "Aceasta este o simulare — niciun cost real.",
    confirmButton: "Confirmă",
    cancelButton: "Anulează",
    successMessage: "Planul a fost schimbat cu succes!",
    profileLabel: "Abonament",
    profileCurrent: "Planul tău curent",
    changePlan: "Schimbă planul",
  },

  apia: {
    title: "APIA — Subvenții și Parcele",
    subtitle: "Gestionează datele APIA, calculează subvenții și exportă rapoarte",
    tabs: {
      parcels: "Parcele",
      calendar: "Calendar",
      calculator: "Calculator",
      export: "Export",
    },
    parcels: {
      title: "Registrul Fermier",
      desc: "Parcele declarate la APIA cu date cadastrale",
      empty: "Nu ai parcele APIA înregistrate",
      emptyDesc: "Adaugă date APIA pentru terenurile tale pentru a ține evidența și calcula subvențiile.",
      addNew: "Adaugă Date APIA",
      edit: "Editează",
      delete: "Șterge",
      confirmDelete: "Sigur vrei să ștergi datele APIA pentru acest teren?",
      registered: "înregistrate",
      totalArea: "Suprafață totală",
      withEco: "Cu eco-schemă",
      form: {
        title: "Date APIA pentru Parcelă",
        selectLand: "Selectează terenul",
        tarlaNumber: "Număr tarlaua",
        parcelNumber: "Număr parcelă",
        sirutaCode: "Cod SIRUTA",
        cadastralNumber: "Număr cadastral",
        landCategory: "Categorie teren",
        apiaArea: "Suprafață declarată APIA (ha)",
        ecoScheme: "Participă la eco-schemă",
        ecoSchemeType: "Tip eco-schemă",
        youngFarmer: "Tânăr fermier (sub 41 ani)",
        notes: "Observații",
        save: "Salvează",
        cancel: "Anulează",
      },
      categories: {
        arabil: "Arabil",
        pasune: "Pășune",
        faneata: "Fâneață",
        livada: "Livadă",
        vie: "Vie",
        legume: "Legume",
      },
      ecoTypes: {
        biodiversitate: "Biodiversitate",
        reducere_pesticide: "Reducere Pesticide",
        pajisti: "Pajiști și Fânețe",
        conversia_eco: "Conversie Ecologică",
        rotatie_culturi: "Rotația Culturilor",
      },
    },
    calendar: {
      title: "Calendar APIA",
      desc: "Termene importante și evenimente ale campaniei APIA",
      daysLeft: "zile rămase",
      daysAgo: "zile în urmă",
      today: "Astăzi",
      upcoming: "Următorul termen",
      noUpcoming: "Nici un termen în viitor",
      priority: {
        HIGH: "Urgent",
        MEDIUM: "Important",
        LOW: "Informativ",
      },
      category: {
        SUBMISSION: "Depunere",
        PAYMENT: "Plată",
        CONTROL: "Control",
        PREPARATION: "Pregătire",
      },
    },
    calculator: {
      title: "Calculator Subvenții",
      desc: "Estimare plăți APIA bazată pe parcele și culturi",
      totalEur: "Total Estimat (EUR)",
      totalRon: "Total Estimat (RON)",
      eligibleParcels: "Parcele Eligibile",
      totalArea: "Suprafață Totală",
      breakdown: "Detaliere",
      perHa: "/ha",
      disclaimer: "Estimare orientativă — valorile finale sunt stabilite de APIA și pot diferi.",
      noParcels: "Adaugă parcele APIA pentru a calcula subvențiile.",
      year: "Campanie",
      exchangeRate: "Curs EUR/RON",
      components: {
        SAPS: "SAPS (Plata pe suprafață)",
        REDISTRIBUTIVE: "Plata Redistributivă",
        ECO_SCHEME: "Eco-schemă",
        COUPLED_SUPPORT: "Sprijin Cuplat",
        YOUNG_FARMER: "Tânăr Fermier",
      },
    },
    export: {
      title: "Export Raport APIA",
      desc: "Generează PDF cu toate datele APIA formatate",
      download: "Descarcă PDF",
      downloading: "Se generează...",
      preview: "Previzualizare conținut",
      includes: "Raportul include:",
      includesList: {
        farmerData: "Datele fermierului",
        parcelDetails: "Lista parcelelor cu detalii cadastrale",
        areas: "Suprafețe și culturi declarate",
        subsidyEstimate: "Estimare subvenții per parcelă",
        totals: "Totaluri generale EUR / RON",
      },
    },
  },
  ui: {
    theme: {
      switchToDark: "Comutați la tema întunecată",
      switchToLight: "Comutați la tema deschisă",
      dark: "Întunecat",
      light: "Deschis",
    },
    navigation: {
      landingPage: "Pagina Principală",
      dashboard: "Tablou de bord",
      lands: "Terenuri",
      alerts: "Alerte",
      economics: "Economie",
      plan: "Plan",
      profile: "Profil",
    },
    premium: {
      unlockFeatures: "Deblocați Funcționalitățile Premium",
      upgrade: "Actualizați Planul",
      learnMore: "Aflați mai multe",
    },
    actions: {
      save: "Salvează",
      cancel: "Anulează",
      edit: "Editează",
      delete: "Șterge",
      update: "Actualizează",
      refresh: "Reîmprospătează",
      close: "Închide",
    },
    status: {
      loading: "Se încarcă...",
      error: "Eroare",
      success: "Succes",
      warning: "Avertisment",
      info: "Informații",
    },
  },
  ai: {
    title: "AgriOne Assistant",
    subtitle: "Expert în agricultură",
    placeholder: "Întreabă ceva despre agricultură...",
    newConversation: "Conversație nouă",
    messagesRemaining: "mesaje rămase azi",
    welcome: {
      title: "Bună! Sunt asistentul tău agricol",
      description: "Întreabă-mă orice despre culturi, boli, tratamente sau sfaturi pentru fermă.",
    },
    quickActions: {
      weather: "Cum îmi afectează vremea culturile?",
      diseases: "Cum previn bolile plantelor?",
      irrigation: "Sfaturi pentru irigație",
      pests: "Control dăunători ecologic",
    },
    errors: {
      processing: "Nu am putut procesa mesajul.",
      imageAnalysis: "Nu am putut analiza imaginea.",
      loadConversation: "Nu am putut încărca conversația.",
      deleteConversation: "Nu am putut șterge conversația.",
      rateLimit: "Ai atins limita de mesaje AI pentru azi.",
    },
    upgradePrompt: "Actualizează la Pro pentru acces complet la AI Assistant",
  },
};
